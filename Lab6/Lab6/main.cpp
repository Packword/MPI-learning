...
int main(int argc, char* argv[]) {
    ...
    for (int iter = 0; iter < procNum; iter++) {
        ...
        if (procNum > 1 && iter != procNum - 1) {
            for (int i = 0; i < size; i++) {
                MPI_Sendrecv_replace(curColumn[i], width, MPI_INT, (procRank + 1) % procNum, 0, procRank == 0 ? procNum - 1 : procRank - 1, MPI_ANY_TAG, SquareComm, &Status);
            }
        }
    ...
    if (procRank == 0) {
        double t2 = MPI_Wtime();
        double res = t2 - t1;
        if(size < 10)print_matrix(matrix_C, size, size);
        std::cout << "\ntime: " << res;
    }
